 ...
 * Handles:
 * - Game board display with modern neumorphic styling
 * - User interactions via clickable columns
...
public class Connect4UI extends JFrame {
    ...
    public enum GameDifficulty {
        ...
        HARD("Hard", "Recursive Priority Filter"),
        EXPERT("Expert", "Threat Lattice Analysis");
    ...
    private void showGameModeDialog() {
        ...
        modeButtonPanel.setBackground(COLOR_BG);
    ...
    private void updateDifficultyLabels() {
        // Update any difficulty-related UI if needed
    }
    ...
    private void showAlgorithmInfo() {
        String info = "Connect 4 - Algorithm Implementation\n\n" +
                ...
                "4. Connected Count (OPTIMIZED)\n" +
                "   - Piece-centric approach O(P) instead of O(R*C)\n" +
                "5. Find Valid Moves\n" +
                "   - Recursively finds valid columns\n" +
                "6. Recursive Winner Detection\n" +
                "   - Checks row/col segments recursively\n" +
                "7. State-Space D&C (Moderate)\n" +
                "   - Standard Minimax foundation\n" +
                "8. Recursive Priority Filter (Hard)\n" +
                "   - Smart filtering: Win > Block > Safety > Center > Connections\n" +
                "9. Threat Lattice Analysis (Expert)\n" +
                "   - Divides board spatially, analyzes threats in each half\n\n" +
                "All difficulty levels utilize these optimized D&C algorithms.";
    ...
    private void handleColumnClick(int col) {
        ...
        if (playWithAI && currentPlayer == 'Y') {
            ...
            // Use SwingWorker to run AI in background - FAST response
            SwingWorker<Integer, Void> aiWorker = new SwingWorker<Integer, Void>() {
                @Override
                protected Integer doInBackground() {
                    // Minimal delay for snappy response
                    try {
                        Thread.sleep(150); // Reduced from 800ms to 150ms
    ...
    private int getAIMoveByDifficulty(char player) {
        switch (difficulty) {
            ...
            case HARD:
                // Recursive Priority Filter (Algo 8)
                return winChecker.findBestMoveRecursiveFilter(player);

            case EXPERT:
                // Threat Lattice Analysis (Algo 9)
                return winChecker.findBestMoveThreatLattice(player);
